...
import io.agora.uikit.impl.users.AgoraUIRosterPopUp
...
class AgoraUISmallClassContainer(
    ...
    private var isFirstTimeInClassRoom = true
    ...
    override fun init(layout: ViewGroup, left: Int, top: Int, width: Int, height: Int) {//layout: contentLayout
        ...
        handsUpPopup?.let { popup ->
            popup.setEduContext(getEduContext())
            popup.initView(layout, optionPopupRight, optionPopupBottom + 12)// layout:contentLayout
            popup.closeRunnable = Runnable {
                optionLayout?.setActivated(OptionItem.HandsUp, false)
            }
        }

        optionLayout = OptionLayout(layout, optionRight, optionBottom,
            object : OptionWindowListener {
                override fun onWindowShow(item: OptionItem, view: View) {
                    when (item) {
                        OptionItem.Setting -> {
                        ...
                        OptionItem.HandsUp -> {
                            if (isFirstTimeInClassRoom){
                                showHandsUpToastPopup()
                                isFirstTimeInClassRoom = false
                            }
                        }
                        else -> {
