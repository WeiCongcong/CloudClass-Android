...
class OptionLayout(private val parent: ViewGroup,
    ...
    init {
        ...
        handsUp.setOnClickListener(this)
        ...
        layout.viewTreeObserver.addOnGlobalLayoutListener(object : ViewTreeObserver.OnGlobalLayoutListener {
            override fun onGlobalLayout() {
                if (layout.width > 0 && layout.height > 0) {
                    ...
                    var timerW = (handsUp.width * 2 / 3f).toInt()
                    timerW = (timerW * 23 / 21f).toInt()
                    val timerH = timerW
                    val timerRight = right + (handsUp.width - timerW) / 2
                    handsUpWrapper = if (AgoraUIConfig.isLargeScreen) {
                        AgoraUIHandsUpWrapper(parent, eduContext,
                                handsUp, timerW, timerH, timerRight, bottom + 11, handsUpPopup)
                    } else {
                        AgoraUIHandsUpWrapper(parent, eduContext,
                                handsUp, timerW, timerH, timerRight, bottom + 20, handsUpPopup)
                    }
                }
            }
        })
    ...
    override fun onClick(v: View?) {
        when (v?.id) {
            setting.id -> {
                ...
                setActivated(OptionItem.HandsUp, false)
                windowListener?.onWindowDismiss(OptionItem.HandsUp)
                ...
                if (isActivated(OptionItem.Setting)) {
                    ...
                    windowListener?.onWindowShow(OptionItem.Setting,v)
            ...
            roster.id -> {
                ...
                if (isActivated(OptionItem.Roster)) {
                    ...
                    windowListener?.onWindowShow(OptionItem.Roster,v)
            ...
            chat.id -> {
                ...
                if (isActivated(OptionItem.Chat)) {
                    ...
                    windowListener?.onWindowShow(OptionItem.Chat,v)
            ...
            handsUp.id -> {
                ...
                windowListener?.onWindowDismiss(OptionItem.Roster)
                windowListener?.onWindowDismiss(OptionItem.Chat)
                if (isActivated(OptionItem.HandsUp)) {
                    windowListener?.onWindowDismiss(OptionItem.HandsUp)
                    setActivated(OptionItem.HandsUp, false)
                } else {
                    windowListener?.onWindowShow(OptionItem.HandsUp,v)
                    setActivated(OptionItem.HandsUp)
                }
            }
            upload.id -> {
    ...
    fun setActivated(item: OptionItem, isActivated: Boolean = true) {
        when (item) {
            ...
            OptionItem.HandsUp -> {
                handsUp.isActivated = isActivated
            }
            else -> {
    ...
    private fun isActivated(item: OptionItem): Boolean {
        return when (item) {
            ...
            OptionItem.HandsUp -> handsUp.isActivated
            else -> false
        }
    }
...
interface OptionWindowListener {
    fun onWindowShow(item: OptionItem, view: View)
