...
class AgoraUIRoomStatusOne2One(parent: ViewGroup,
    ...
    private val contentView: View = LayoutInflater.from(parent.context).inflate(
        R.layout.agora_status_bar_layout_one2one, parent, false)
    ...
    private fun showDeviceSettingPopUp(parent: ViewGroup) {
        popupView = AgoraUIDeviceSettingPopUp(parent.context)
        ...
        (popupView as? AgoraUIDeviceSettingPopUp)?.setEduContextPool(eduContext)
        (popupView as? AgoraUIDeviceSettingPopUp)?.leaveRoomRunnable = Runnable {
            OptionsLayout.listener?.onLeave()
            settingBtn.isActivated = false

        }
        (popupView as? AgoraUIDeviceSettingPopUp)?.initView(parent, windowWidth, windowHeight)
        val windowLeft = (parent.width * 0.7).toInt()
        val windowTop = (parent.height * 0.06).toInt()
        val rect = Rect(windowLeft, windowTop, windowLeft + windowWidth, windowTop + windowHeight)
        ...
        parent.addView(popupView, param)
    }
    ...
    private fun updateConnectionState(connectionState: EduContextConnectionState) {
        if (connectionState == EduContextConnectionState.Aborted) {
            AgoraUIToast.error(context = contentView.context, text = contentView.context.resources
