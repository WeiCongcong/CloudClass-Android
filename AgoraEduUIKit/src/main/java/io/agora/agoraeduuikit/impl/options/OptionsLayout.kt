...
//    }
    ...
    fun init(eduContext: EduContextPool?, parent: RelativeLayout, role: AgoraEduContextUserRole,
        ...
        if (roomType == EduContextRoomType.SmallClass ||
            roomType == EduContextRoomType.SmallClassArt ||
            roomType == EduContextRoomType.LargeClass) {
            if (isTeacher) {
                ...
                handsUpItem?.let {
                    addHandsUpButton(it, width, R.drawable.agora_handsup_down_img_art)
                }
                ...
                addHandsUpButton(item, width, R.drawable.agora_handsup_down_img_art)
            }
        }
        ...
        // if localUser is teacher, show whiteBoardItem, otherwise hide.
        grantPermission(this.role == AgoraEduContextUserRole.Teacher)
    ...
    private fun addHandsUpButton(item: OptionsLayoutItem, width: Int, res: Int) {
        item.setOnClickListener(this)
        addView(item, width, width)
    }
    ...
    private fun addWhiteboardToolButton(parent: RelativeLayout,
        ...
        OptionsLayoutWhiteboardItem(parent, size, popupMargin, eduContext).let {
            boardToolItem = it
...
abstract class OptionsLayoutItem(context: Context?,
    ...
    protected val iconViewSizeRatio = 28 / 46f
    protected val iconButtonSizeRatio = 36 / 46f
...
class ChatLayoutPopupItem(private val parent: RelativeLayout,
    ...
    private val widgetMessageObserver = object : AgoraWidgetMessageObserver {
        override fun onMessageReceived(msg: String, id: String) {
            ...
            if (packet.signal == UnreadTips && packet.body is Boolean) {
                if (getMiddleLayer().isVisible) {
                    redDot?.visibility = GONE
                    return
                }
                redDot?.post { redDot?.visibility = if (packet.body) VISIBLE else GONE }
            }
        }
    }
    ...
    override fun onPopupRect(): Rect {
        ...
        var windowHeight = (windowBaseHeight * parent.height / containerBaseHeight).toInt()

        // Adjust the window size by scaling it according to the
        // ratio of design sizes for different screen size
        if (AgoraUIConfig.isLargeScreen) {
            windowHeight = (windowHeight * 430 / 287f).toInt()
        }
        ...
        override fun onMessageReceived(msg: String, id: String) {
            ...
            when (packet.signal) {
                AgoraBoardInteractionSignal.BoardGrantDataChanged -> {
                    eduContext?.userContext()?.getLocalUserInfo()?.userUuid?.let { userUuid ->
                        val granted = (packet.body as? ArrayList<String>)?.contains(userUuid)
                            ?: false
                        this@OptionsLayoutWhiteboardItem.post {
                            this@OptionsLayoutWhiteboardItem.isVisible = granted
                        }
                    }
                }
                MemberStateChanged -> {
