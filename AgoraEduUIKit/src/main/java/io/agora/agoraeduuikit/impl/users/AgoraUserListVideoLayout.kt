...
//    }
    ...
    val uiDataProviderListener = object : UIDataProviderListenerImpl() {
        ...
        override fun onUserHandsWave(userInfo: AgoraEduContextUserInfo, duration: Int?) {
            super.onUserHandsWave(userInfo, duration)
            updateUserWaveState(userInfo.userUuid, true)
        }

        override fun onUserHandsDown(userInfo: AgoraEduContextUserInfo) {
            super.onUserHandsDown(userInfo)
            updateUserWaveState(userInfo.userUuid, false)
        }
    }
    ...
    private fun render(viewGroup: ViewGroup?, streamUuid: String) {
        ...
        if (noneView && isLocal) {
            eduContext?.mediaContext()?.stopRenderLocalVideo(streamUuid)
        } else if (noneView && !isLocal) {
            eduContext?.mediaContext()?.stopRenderLocalVideo(streamUuid)
        } else if (!noneView && isLocal) {
            eduContext?.mediaContext()?.startRenderLocalVideo(EduContextRenderConfig(), viewGroup!!, streamUuid)
        } else if (!noneView && !isLocal) {
            eduContext?.mediaContext()?.startRenderRemoteVideo(EduContextRenderConfig(), viewGroup!!, streamUuid)
        }
    }

    private fun isLocalStream(streamUuid: String): Boolean {
        val localUserUuid = localUserInfo?.userUuid
        val info = eduContext?.streamContext()?.getAllStreamInfo()?.find {
        ...
        rv.addItemDecoration(object : RecyclerView.ItemDecoration() {
            override fun getItemOffsets(outRect: Rect, view: View, parent: RecyclerView, state: RecyclerView.State) {
                ...
                val roomType = eduContext?.roomContext()?.getRoomInfo()?.roomType
                        ?: EduContextRoomType.LargeClass
                if (roomType == EduContextRoomType.LargeClass) {
    ...
    private fun updateCoHostList(list: MutableList<AgoraUIUserDetailInfo>) {
        if (list.size > 0) {
            ...
            if (eduContext?.roomContext()?.getRoomInfo()?.roomType == EduContextRoomType.LargeClass) {
                ...
                val item = list.find { it.userUuid == userInfo?.userUuid }
                item?.let {
                    list.remove(it)
                    list.add(0, it)
                }
