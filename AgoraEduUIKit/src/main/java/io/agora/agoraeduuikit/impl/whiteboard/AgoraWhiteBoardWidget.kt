...
//                }
    ...
    var boardParams: BoardParams? = null
...
//        whiteBoardView.setBackgroundColor(0)
    ...
    private fun joinWhiteboard(uuid: String, boardToken: String) {
        ...
        boardParams?.let {
            configuration.useMultiViews = it.useMultiViews
            if (configuration.useMultiViews) {
                setMultiViewProperties(params, it)
            }
        }
    ...
    private fun setMultiViewProperties(params: RoomParams, boardParams: BoardParams) {
        boardParams.styleParams?.let {
            val collectorStyleMap = HashMap<String, String>()
            collectorStyleMap["position"] = "fixed"
            collectorStyleMap["left"] = it.left.toString() + "px"
            collectorStyleMap["bottom"] = it.bottom.toString() + "px"
            params.windowParams.setCollectorStyles(collectorStyleMap)
        }
...
//        val extra = widgetInfo?.properties?.get(grantUser) as? MutableMap<*, *>
        val tmp = widgetInfo?.properties?.get(grantUser) as? ArrayList<*>
