...
class UIDataProvider(private val eduContext: EduContextPool?) {
    ...
    private val listeners = mutableListOf<UIDataProviderListener>()

    @Volatile
    private var grantedUsers = mutableListOf<String>()
    ...
    private val widgetObserver = object : AgoraWidgetMessageObserver {
        override fun onMessageReceived(msg: String, id: String) {
            ...
            if (id == AgoraWidgetDefaultId.WhiteBoard.id && packet.body is MutableList<*> &&
                packet.signal == AgoraBoardInteractionSignal.BoardGrantDataChanged) {
                grantedUsers.clear()
    ...
    private val providerStreamHandler = object : StreamHandler() {
        ...
        override fun onStreamUpdated(streamInfo: AgoraEduContextStreamInfo,
                                     operator: AgoraEduContextUserInfo?) {
            if (streamInfo.videoSourceType == AgoraEduContextVideoSourceType.Camera) {
                callbackBothKindsOfUserList()
            } else if (streamInfo.videoSourceType == AgoraEduContextVideoSourceType.Screen) {
        ...
        override fun onStreamLeft(streamInfo: AgoraEduContextStreamInfo,
                                  operator: AgoraEduContextUserInfo?) {
            if (streamInfo.videoSourceType == AgoraEduContextVideoSourceType.Camera) {
                callbackBothKindsOfUserList()
            } else if (streamInfo.videoSourceType == AgoraEduContextVideoSourceType.Screen) {
    ...
    private val providerMediaHandler = object : MediaHandler3() {
    ...
    private fun callbackScreenShare(streamInfo: AgoraEduContextStreamInfo, stop: Boolean = false) {
        eduContext?.userContext()?.getUserList(streamInfo.owner.role)?.find {
            ...
            val userDetailInfo = toAgoraUserDetailInfo(it,
                coHostSet.contains(it.userUuid), streamInfo)
            iterateListeners { listener ->
                if (stop) {
    ...
    private fun callbackBothKindsOfUserList(localAudioState: AgoraEduContextDeviceState2? = null,
    ...
    private fun callbackCoHostUserList(localAudioState: AgoraEduContextDeviceState2? = null,
                                       localVideoState: AgoraEduContextDeviceState2? = null) {
    ...
    fun notifyUserListChanged(localAudioState: AgoraEduContextDeviceState2? = null,
                              localVideoState: AgoraEduContextDeviceState2? = null) {
    ...
    fun notifyScreenShareDisplay() {
        eduContext?.streamContext()?.getAllStreamInfo()?.forEach { streamInfo ->
    ...
    private fun toAgoraUserDetailInfo(list: List<AgoraEduContextUserInfo>,
                                      localAudioState: AgoraEduContextDeviceState2? = null,
                                      localVideoState: AgoraEduContextDeviceState2? = null): List<AgoraUIUserDetailInfo> {
        ...
        eduContext?.let { context ->
            ...
            list.forEach { userInfo ->
                val streamInfoList = context.streamContext()?.getStreamInfo(userInfo.userUuid)
                val streamInfo = if (streamInfoList != null && streamInfoList.isNotEmpty())
                    streamInfoList[0] else null
                userList.add(
                    if (userInfo.userUuid == localUserUuid) {
                        toAgoraUserDetailInfo(userInfo,
    ...
    private fun toAgoraUserDetailInfo(user: AgoraEduContextUserInfo,
                                      isCoHost: Boolean,
                                      stream: AgoraEduContextStreamInfo? = null,
                                      localAudioState: AgoraEduContextDeviceState2? = null,
                                      localVideoState: AgoraEduContextDeviceState2? = null): AgoraUIUserDetailInfo {
...
open class UIDataProviderListenerImpl : UIDataProviderListener {
    ...
    override fun onUserHandsWave(userInfo: AgoraEduContextUserInfo, duration: Int?) {

    }

    override fun onUserHandsDown(userInfo: AgoraEduContextUserInfo) {
...
data class AgoraUIUserDetailInfo(
    ...
    val audioSourceState: AgoraEduContextMediaSourceState,
    val videoSourceState: AgoraEduContextMediaSourceState) {

    fun copy(): AgoraUIUserDetailInfo {
        return AgoraUIUserDetailInfo(userUuid, userName, role, isCoHost, reward, whiteBoardGranted,
