...
replaceRte() {
    ...
    if [ $1 = "arsenal" ]; then
        cp -f -r ./rte-arsenal ./edu/src/main/java/io/agora/edu/core/internal/rte
        if [ $? -eq 0 ]; then
    ...
    # use old rtc to build project
        ...
        cp -f -r ./rte-rtc ./edu/src/main/java/io/agora/edu/core/internal/rte
        if [ $? -eq 0 ]; then
...
updateGradlePropertyAndSync() {
    ...
    ./gradlew -b replaceRte.gradle -PUSE_ARSENAL=$1
    if [ $? -eq 0 ]; then
...
if [ $emptyParameter -eq 0 ]; then
    ...
    rm -f -r ./edu/src/main/java/io/agora/edu/core/internal/rte
    if [ $? -eq 0 ]; then
