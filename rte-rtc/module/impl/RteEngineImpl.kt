...
import io.agora.edu.core.internal.rte.module.RteEngine.Companion.getErrorDescription
...
object RteEngineImpl : IRteEngine, IRtmServerDelegate {
            ...
            speakers?.let {
                ...
                if (speakers.size == 1 && speakers[0].uid == 0) {
                    ...
                    rteChannelEventListenerMap.forEach {
                        it.value.onAudioVolumeIndicationOfLocalSpeaker(rteSpeakers, totalVolume)
                    }
                } else {
                    speakerReportListener?.onAudioVolumeIndicationOfRemoteSpeaker(rteSpeakers, totalVolume)
                    rteChannelEventListenerMap.forEach {
                        it.value.onAudioVolumeIndicationOfRemoteSpeaker(rteSpeakers, totalVolume)
                    }
                }
            }
        }
    ...
    override fun init(context: Context, appId: String, logFileDir: String, rtcRegion: String?,
        ...
        rtcEngineEx = RteEngine.create(config)
    ...
    override fun setLatencyLevel(channelId: String, level: Int) {
        if (channelMap.isEmpty()) {
            AgoraLog.e("$tag:channelMap is Empty, failed to set LatencyLevel ->$level")
            return
        }
        (channelMap[channelId] as RteChannelImpl).latencyLevel = level
    }
    ...
    override fun setClientRole(channelId: String, uid: String, role: Int): Int {
        ...
        val code = (channelMap[channelId] as RteChannelImpl).setClientRole(uid, role)
        if (code == 0) {
    ...
    override fun publish(channelId: String, uid: String): Int {
        if (channelMap.isNotEmpty()) {
            val code0 = (channelMap[channelId] as RteChannelImpl).publish(uid)
            if (code0 != ok()) {
                return code0
            }
    ...
    override fun unpublish(channelId: String, uid: String): Int {
        if (channelMap.isNotEmpty()) {
            val code0 = (channelMap[channelId] as RteChannelImpl).unPublish(uid)
            if (code0 != ok()) {
                return code0
            }
    ...
    override fun updateLocalStream(channelId: String, uid: String, hasAudio: Boolean, hasVideo: Boolean): Int {
        ...
        val c = (channelMap[channelId] as RteChannelImpl).muteLocalAudioStream(uid, !hasAudio)
        if (c != ok()) {
            return c
        }
        val d = (channelMap[channelId] as RteChannelImpl).muteLocalVideoStream(uid, !hasVideo)
        if (d != ok()) {
            return d
        }
        return ok()
    ...
    override fun muteRemoteStream(channelId: String, uid: String, muteAudio: Boolean, muteVideo: Boolean): Int {
        if (channelMap.isNotEmpty()) {
            ...
            return channel.muteRemoteStream(uid, muteAudio, muteVideo)
        }
        return -1
    }

    override fun muteLocalStream(channelId: String, uid: String, muteAudio: Boolean, muteVideo: Boolean): Int {
        return (channelMap[channelId] as RteChannelImpl).muteLocalStream(uid, muteAudio, muteVideo)
    }

    override fun muteLocalAudioStream(channelId: String, uid: String, muteAudio: Boolean): Int {
        return (channelMap[channelId] as RteChannelImpl).muteLocalAudioStream(uid, muteAudio)
    }

    override fun muteLocalVideoStream(channelId: String, uid: String, muteVideo: Boolean): Int {
        return (channelMap[channelId] as RteChannelImpl).muteLocalVideoStream(uid, muteVideo)
    }
    ...
    override fun enableAudioVolumeIndication(interval: Int, smooth: Int, report_vad: Boolean) {
        ...
        rtcEngineEx.enableAudioVolumeIndication(interval, smooth, report_vad)
    }
    // TODO 1
    ...
    override fun setChannelMode(channelId: String, curChannelLocalUid: String, mode: Int): Int {
        if (channelMap.isNotEmpty()) {
            val channel = channelMap[channelId] as RteChannelImpl
            return channel.setChannelMode(curChannelLocalUid, mode)
        }
        return -1
    }
