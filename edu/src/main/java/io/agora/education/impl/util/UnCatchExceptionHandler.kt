...
class UnCatchExceptionHandler : Thread.UncaughtExceptionHandler {
    ...
    companion object {
        ...
        fun hasException(): Boolean {
            unCatchExceptionHandler.context?.let {
                return SharedPreferenceManager.instance().get(it, ANDROID_EXCEPTION, false)
            }
            return false
        }
    ...
    private fun handlerException(t: Thread?, e: Throwable?): Boolean {
        e?.let {
            ...
            context?.let {
                SharedPreferenceManager.instance().put(it, ANDROID_EXCEPTION, true)
            }
    ...
    fun uploadAndroidException() {
        val uploadParam = UploadManager.UploadParam(
        ...
        AgoraLog.i("$tag: Call the uploadLog function to upload logs when " +
                "handleUnCatchException，parameter ->${Gson().toJson(uploadParam)}")

        context?.let { context ->
            UploadManager.upload(context, Constants.APPID, BuildConfig.LOG_OSS_CALLBACK_HOST,
                logDir, uploadParam, object : ThrowableCallback<String> {
                    override fun onSuccess(res: String?) {
                        res?.let {
                            AgoraLog.e("$tag: Log uploaded successfully->$res")
                            SharedPreferenceManager.instance().put(context, ANDROID_EXCEPTION, false)
                        }
                    }
