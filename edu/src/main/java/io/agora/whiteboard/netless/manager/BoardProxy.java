...
public class BoardProxy extends NetlessManager<Room> implements RoomCallbacks {
    ...
    public void init(WhiteSdk sdk, RoomParams params) {
        ...
        AgoraLog.i(TAG + "->joinRoom: params:" + new Gson().toJson(params));
...
//        }
    ...
    public void setWritable(boolean writable) {
        if (t != null) {
            AgoraLog.i(TAG + "->setWritable:" + writable);
    ...
    void onSuccess(Room room) {
        Log.e(TAG, "onSuccess:");
        AgoraLog.i(TAG + "->onSuccess: roomParams:" + new Gson().toJson(roomParams));
        if (appliance != null) {
        ...
        if (writable != null) {
            AgoraLog.i(TAG + "->onSuccess: try to restore setWritable:" + writable);
    ...
    void onFail(SDKError error) {
        ...
        if (error.toString().contains(sdkInitFailed) && joinFailedRetry > 0) {
            ...
            AgoraLog.e(TAG + "->joinRoom-retry-sdkInitFailed");
            ...
            AgoraLog.e(TAG + "->joinRoom-retry-magixConnectFailed");
            this.whiteSdk.joinRoom(this.roomParams, this, promise);
            connectFailedRetry--;
            return;
        }
