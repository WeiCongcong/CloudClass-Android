...
abstract class BaseClassActivity : BaseActivity(),
    ...
    private val handsUpContext = object : HandsUpContext() {
        override fun performHandsUp(state: EduContextHandsUpState, callback: EduContextCallback<Boolean>?, timeout:Int) {
            handsUpManager?.performHandsUp(state, callback, timeout)
        }

        override fun performHandsWave(state: EduContextHandsUpState, callback: EduContextCallback<Boolean>?, timeout: Int) {
            handsUpManager?.performHandsWave(state, callback, timeout)
        }
    }
    ...
    override fun onStart() {
        super.onStart()
        launchConfig?.let { EyeProtection.setNeedShow(this, it.eyeCare == 1) }
    }
