...
import io.agora.edu.common.bean.handsup.HandsUpConfig.Companion.handsWaveKey
...
open class UserListManager(
    ...
    private fun parseHandsUpConfig2(): HandsUpConfig? {
        val processesJson = getProperty(eduRoom?.roomProperties, processesKey)
        val processesMap: MutableMap<String, Any>? = Gson().fromJson(processesJson, object : TypeToken<MutableMap<String, Any>>() {}.type)
        val handsUpJson = getProperty(processesMap, handsWaveKey)
        val handsUpConfig = Gson().fromJson(handsUpJson, HandsUpConfig::class.java)
        return handsUpConfig
    }
    ...
    fun notifyUserList() {
        ...
        val handsWaveConfig = parseHandsUpConfig2()
        if (handsWaveConfig == null) {
            Constants.AgoraLog.e("$tag->handsWaveConfig is null!")
        }
        val wavingList: MutableList<String> = mutableListOf()//正在挥手的学生列表
        handsWaveConfig?.progress?.forEach {// 拿到正在举手的学生
            wavingList.add(it.userUuid)
        }
        ...
        getCurRoomFullUser(object : EduCallback<MutableList<EduUserInfo>> {
            override fun onSuccess(userInfos: MutableList<EduUserInfo>?) {
                ...
                userInfos.forEach { element ->
                    ...
                    userDetailInfo.isWaving = wavingList.contains(element.userUuid)
    ...
    fun notifyListByPropertiesChanged(cause: MutableMap<String, Any>?) {
        if (cause != null && cause.isNotEmpty()) {
            ...
            if (causeType == PropertyData.COVIDEO_CHANGED) {
                ...
                if ( data.actionType == HandsUpAction.StudentApply.value|| data.actionType == HandsUpAction.TeacherAccept.value ||
                        data.actionType == HandsUpAction.TeacherAbort.value || data.actionType == HandsUpAction.TeacherTimeout.value ||
