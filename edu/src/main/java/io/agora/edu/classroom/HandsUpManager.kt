                    ...
                    HandsUpAction.TeacherTimeout.value -> {
                        ...
                        data?.removeProgress?.forEach {
                            if (it.userUuid == localUserUuid) {
                                ...
                                state = AgoraUIHandsUpState.HandsDown
                                handsUpContext?.getHandlers()?.forEach { h ->
//                                    h.onHandsUpTips(context.getString(R.string.handsuptimeout))//timeout toast
                                    h.onHandsUpStateUpdated(EduContextHandsUpState.HandsDown, coHost)
                                }
                            }
                        }
    ...
    private fun handsUp(callback: EduContextCallback<Boolean>? = null, timeout: Int) {
    ...
    private fun handsUpWave(callback: EduContextCallback<Boolean>? = null, timeout: Int) {
        handsUp.applyHandsWave(object : EduCallback<Boolean> {
            override fun onSuccess(res: Boolean?) {
                res?.let { success ->
                    if (success) {
                        callback?.onSuccess(true)
    ...
    fun performHandsUp(state: EduContextHandsUpState, callback: EduContextCallback<Boolean>?, timeout: Int) {
        if (state == EduContextHandsUpState.HandsUp) {
            handsUp(callback, timeout)
        } else if (state == EduContextHandsUpState.HandsDown) {
            handsDown(callback)
        }
    }

    fun performHandsWave(state: EduContextHandsUpState, callback: EduContextCallback<Boolean>?, timeout: Int) {
        if (state == EduContextHandsUpState.HandsUp) {
            handsUpWave(callback, timeout)//jieshou callback
        }
    }
}
