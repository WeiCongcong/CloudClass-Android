...
abstract class AgoraExtAppBase : IAgoraExtApp {
    ...
    private val valueDeleted = "deleted"
...
//        tryInitializedPositionBySync()
    }

    override fun onPropertyUpdated(properties: MutableMap<String, Any?>?, cause: MutableMap<String, Any?>?) {
        val causeBean = Gson().fromJson(Gson().toJson(cause), AgoraExtAppCause::class.java)
        if (causeBean != null && causeBean.cmd == AgoraExtAppCauseCMD.PositionChanged.value) {
            val positionJson = Gson().toJson(properties?.get(keyExtAppPosition))
            val position = Gson().fromJson(positionJson, AgoraExtAppMovement::class.java)
            position?.let {
                onPositionSync(it.userId, it.x, it.y)
                return
            }
        }
    }
        ...
        parent?.let { parent ->
            layout?.let { layout ->
                if (layout.parent == parent) {
                    ContextCompat.getMainExecutor(layout.context).execute {
                        ...
                        if (fix) {
                            ...
                            val left = (parent.width - layout.width)
                            param.leftMargin = left / 2
                            val top = (parent.height - layout.height)
                            param.topMargin = top / 2
                            Log.e(tag, "位置->left:${param.leftMargin}, top:${param.topMargin}")
                        }
    ...
    private fun tryInitializedPositionBySync() {
        val properties = extAppContext?.properties
        val tmp = Gson().toJson(properties?.get(keyExtAppPosition))
        if(tmp.contains(valueDeleted)) {
            // property is deleted
            return
        }
        val movement = Gson().fromJson(tmp,
                AgoraExtAppMovement::class.java)
        movement?.let {
            movePositionBySync(movement.x, movement.y)
        }
        if (!positionInit) positionInit = true
    }

    private fun onPositionSync(userId: String, x: Float, y: Float) {
    ...
    private fun movePositionBySync(x: Float, y: Float) {
        layout?.let { self ->
            parent?.let { parent ->
                ...
                ContextCompat.getMainExecutor(self.context).execute {
...
enum class AgoraExtAppState(val value: Int) {
    Stopped(0),
    Running(1)
}

data class AgoraExtAppMovement(
        val userId: String,
        val x: Float,
        val y: Float)

data class AgoraExtAppCause(
        val cmd: Int,
        val data: Any? = null
)

enum class AgoraExtAppCauseCMD(val value: Int) {
    PositionChanged(0)
}