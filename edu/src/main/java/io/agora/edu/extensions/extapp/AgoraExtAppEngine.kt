...
class AgoraExtAppEngine(
    ...
    companion object {
         ...
         */
        fun formatExtAppIdentifier(id: String): String {
            return id.replace(".", "_")
        }
    }
    ...
    fun launchExtApp(identifier: String, formatted: Boolean = false, currentTime: Long): Int {
        ...
        ContextCompat.getMainExecutor(context).execute {
            ...
            if (item.contentView != null) {
                // fix bug: extApp fly from topLeft to center when loaded(workRound, maybe have another problems)
//                container.addView(item.contentView, RelativeLayout.LayoutParams(
//                        item.param.width, item.param.height))
                val params = RelativeLayout.LayoutParams(item.param.width, item.param.height)
                params.addRule(RelativeLayout.CENTER_IN_PARENT, RelativeLayout.TRUE)
                container.addView(item.contentView, params)
    ...
    internal fun syncAppPosition(identifier: String, userId: String, diffX: Float, diffY: Float) {
        launchedExtAppMap[identifier]?.let { item ->
            aPaaSEntry.syncAppPosition(item.formatIdentifier, userId, diffX, diffY)
        }
    }
