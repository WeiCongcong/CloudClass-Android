...
import io.agora.edu.core.internal.edu.classroom.bean.PropertyData.FLEX
...
import io.agora.edu.core.internal.rte.data.RteLocalAudioState
...
internal open class BaseManager(
    ...
    fun muteRemoteAudio(streamUuid: String, muted: Boolean) {
        eduRoom?.getStreamInfo(streamUuid, object : EduCallback<EduStreamInfo?> {
            override fun onSuccess(res: EduStreamInfo?) {
                res?.let { info ->
                    info.hasAudio = !muted
                    handleStreamMuteWithEmptyCallback(info)
                }
            }
    ...
    fun muteRemoteVideo(streamUuid: String, muted: Boolean) {
        eduRoom?.getStreamInfo(streamUuid, object : EduCallback<EduStreamInfo?> {
            override fun onSuccess(res: EduStreamInfo?) {
                res?.let { info ->
                    info.hasVideo = !muted
    ...
    private fun handleStreamMuteWithEmptyCallback(streamInfo: EduStreamInfo) {
        eduUser.muteStream(streamInfo, object : EduCallback<Boolean> {
    ...
    private fun switchLocalVideoAudio(openVideo: Boolean, openAudio: Boolean) {
        ...
        localStream?.let {
            ...
            eduUser.initOrUpdateLocalStream(
                LocalStreamInitOptions(it.streamUuid, openVideo, openAudio,
