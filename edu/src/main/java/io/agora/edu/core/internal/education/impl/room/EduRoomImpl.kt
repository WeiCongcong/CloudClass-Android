...
import io.agora.edu.core.internal.rte.data.VideoDimensions.VideoDimensions_160X120
import io.agora.edu.core.internal.rte.data.VideoDimensions.VideoDimensions_320X240
...
internal class EduRoomImpl(
        ...
        RetrofitManager.instance()!!.getService(AgoraEduSDK.baseUrl(), UserService::class.java)
                ...
                .enqueue(RetrofitManager.Callback(0, object : ThrowableCallback<DataResponseBody<EduEntryRes>> {
                    override fun onSuccess(res: DataResponseBody<EduEntryRes>?) {
                        ...
                        joinRte(rtcToken, roomEntryRes.user.streamUuid, mediaOptions.convert(),
    ...
    private fun setVideoEncoderConfig(videoEncoderConfig: RteVideoEncoderConfig?) {
        val config = videoEncoderConfig ?: syncSession.localUser.videoEncoderConfig
        val a = RteEngineImpl.setVideoEncoderConfiguration(config)
        if (a != ok()) {
    ...
    private fun joinRte(rtcToken: String, rtcUid: String, options: RteChannelMediaOptions,
                        encryptionConfig: RteEncryptionConfig, joinOptions: RoomJoinOptions,
                        rtcMode: Int, rtcParams: ArrayList<String>, @NonNull callback: RteCallback<Void>) {
        AgoraLog.i("$TAG->join Rtc and Rtm")
        RteEngineImpl.setChannelMode(getCurRoomUuid(), rtcUid, rtcMode)
        rtcParams.forEach {
            RteEngineImpl.setRtcParameters(it)
        }
        RteEngineImpl.setLatencyLevel(getCurRoomUuid(), joinOptions.latencyLevel.value)
        // TODO 这里本来是要设置为CLIENT_ROLE_AUDIENCE的，但是因为rte的bug，先设置为CLIENT_ROLE_BROADCASTER；
        // bug: 观众身份调用enableLocalDevice时返回-8
        RteEngineImpl.setClientRole(getCurRoomUuid(), rtcUid, CLIENT_ROLE_BROADCASTER)
    ...
    private fun handleLocalStream(stream: EduStreamInfo, callback: EduCallback<Unit>?) {
        ...
        syncSession.localUser.initOrUpdateLocalStream(localStreamInitOptions, object : EduCallback<EduStreamInfo> {
            override fun onSuccess(res: EduStreamInfo?) {
                ...
                RteEngineImpl.setClientRole(getCurRoomUuid(), stream.streamUuid, CLIENT_ROLE_BROADCASTER)
                RteEngineImpl.muteLocalStream(getCurRoomUuid(), stream.streamUuid, !stream.hasAudio, !stream.hasVideo)
                RteEngineImpl.publish(getCurRoomUuid(), stream.streamUuid)
    ...
    override fun getStreamInfo(streamUuid: String, callback: EduCallback<EduStreamInfo?>) {
        var info: EduStreamInfo? = null
        for (element in getCurStreamList()) {
            if (element.streamUuid == streamUuid) {
                info = element.copy()
                break
            }
        }
    ...
    override fun getUserInfo(userUuid: String, callback: EduCallback<EduUserInfo?>) {
        var info: EduUserInfo? = null
        for (element in getCurUserList()) {
            if (element.userUuid == userUuid) {
                info = element
                break
            }
        }
...
//            }
    ...
    override fun onRemoteVideoStateChanged(channelId: String, uid: Int, state: Int, reason: Int, elapsed: Int) {
        ...
        getCurLocalUser().eventListener?.onRemoteVideoStateChanged(channelId, uid, state, reason, elapsed)
    }
    ...
    override fun onRemoteAudioStateChanged(channelId: String, uid: Int, state: Int, reason: Int, elapsed: Int) {
        ...
        getCurLocalUser().eventListener?.onRemoteAudioStateChanged(channelId, uid, state, reason, elapsed)
    }
