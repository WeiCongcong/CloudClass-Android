...
import io.agora.edu.core.context.EduContextVideoDimensions.VideoDimensions_320X240
import io.agora.edu.core.internal.rte.data.RteMirrorMode
    ...
    companion object {
        fun fromValue(value: Int): EduContextRoomType {
            return when (value) {
                ...
                SmallClassArt.value -> SmallClassArt
                else -> OneToOne
            }
        }
    }
...
enum class EduContextDeviceLifecycle(val value: Int) {
    // stop capture, release resource
    Stop(0),

    // restore the device state to the state before Stop
    Resume(1)
}
...
data class EduContextRenderConfig(
    ...
    fun convert(): RteRenderConfig {
        val render = when (this.renderMode) {
            EduContextRenderMode.HIDDEN -> RteRenderMode.HIDDEN
            EduContextRenderMode.FIT -> RteRenderMode.FIT
        }
        val mirror = when (this.mirrorMode) {
            EduContextMirrorMode.AUTO -> RteMirrorMode.AUTO
            EduContextMirrorMode.ENABLED -> RteMirrorMode.ENABLED
            EduContextMirrorMode.DISABLED -> RteMirrorMode.DISABLED
        }
        return RteRenderConfig(render, mirror)
    }
}
...
data class EduContextVideoEncoderConfig(
    ...
    fun convert(): RteVideoEncoderConfig {
        return RteVideoEncoderConfig(videoDimensionWidth, videoDimensionHeight, frameRate, bitRate,
                mirrorMode.value)
    }
}
