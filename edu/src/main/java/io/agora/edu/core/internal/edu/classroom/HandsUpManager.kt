...
import io.agora.edu.core.internal.edu.classroom.bean.PropertyData.HANDSUP_ENABLE_CHANGED
import io.agora.edu.core.internal.edu.classroom.bean.PropertyData.COVIDEO_CHANGED
import io.agora.edu.core.internal.edu.common.bean.handsup.*
import io.agora.edu.core.internal.edu.common.impl.HandsUpImpl
...
import io.agora.edu.uikit.impl.handsup.AgoraUIHandsUpState
...
internal class HandsUpManager(
    ...
    fun acceptCoHost(userUuid: String, callback: EduContextCallback<Boolean>? = null) {
        handsUp.acceptCoHost(userUuid, object : EduCallback<Boolean> {
            override fun onSuccess(res: Boolean?) {
                res?.let {
    ...
    fun endCoHost(userUuid: String, callback: EduContextCallback<Boolean>? = null) {
        val isLocal = userUuid == eduUser.userInfo.userUuid
        val callbackInner = object : EduCallback<Boolean> {
            override fun onSuccess(res: Boolean?) {
                res?.let {
                    callback?.onSuccess(true)
                }
        ...
        if (isLocal) {
            handsUp.endCoHost(callbackInner)
        } else {
            handsUp.endCoHost(userUuid, callbackInner)
        }
    }

    fun updateCoHost(userUuid: String, isCoHost: Boolean) {
        if (isCoHost) {
            acceptCoHost(userUuid)
        } else {
            endCoHost(userUuid)
        }
    }

    fun reward(userUuid: String, count: Int) {
        handsUp.reward(userUuid, count, object : EduCallback<Boolean> {
            override fun onSuccess(res: Boolean?) {
    ...
    fun kick(userUuid: String, forever: Boolean) {
        handsUp.kick(userUuid, forever, object : EduCallback<Boolean> {
            override fun onSuccess(res: Boolean?) {
