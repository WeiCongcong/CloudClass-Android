...
import io.agora.edu.core.internal.framework.EduActionConfig
...
import io.agora.edu.core.internal.education.impl.user.data.request.*
import io.agora.edu.core.internal.education.impl.util.Convert
import io.agora.edu.core.internal.education.impl.util.Convert.convertFromUserInfo
import io.agora.edu.core.internal.framework.data.*
import io.agora.edu.core.internal.framework.data.EduError.Companion.noError
import io.agora.edu.core.internal.rte.data.*
import io.agora.edu.core.internal.rte.module.impl.RteEngineImpl
import io.agora.edu.core.internal.rte.module.impl.RteEngineImpl.getError
import io.agora.edu.core.internal.rte.data.RteClientRole.CLIENT_ROLE_AUDIENCE
...
import io.agora.edu.core.internal.server.struct.request.EduRoomMsgReq
import io.agora.edu.core.internal.server.struct.request.EduUserChatMsgReq
import io.agora.edu.core.internal.server.struct.request.EduUserMsgReq
...
internal open class EduUserImpl(
    ...
    override var videoEncoderConfig = RteVideoEncoderConfig()
...
//        }
        ...
        if (options.enableMicrophone != lastMicState || lastMicState == null) {
            ...
            RteEngineImpl.muteLocalAudioStream(eduRoom.getCurRoomUuid(), options.streamUuid,
                    !options.hasAudio)
        ...
        if (options.enableCamera != lastCameraState || lastCameraState == null) {
            ...
            RteEngineImpl.muteLocalVideoStream(eduRoom.getCurRoomUuid(), options.streamUuid,
                    !options.hasVideo)
            if (c2 != ok()) {
    ...
    override fun subscribeStream(stream: EduStreamInfo, options: StreamSubscribeOptions,
        ...
        /**订阅远端流*/
        val code = RteEngineImpl.muteRemoteStream(eduRoom.getCurRoomUuid(), stream.streamUuid,
                !options.subscribeAudio, !options.subscribeVideo)
                ...
                .enqueue(RetrofitManager.Callback(0, object : ThrowableCallback<ResponseBody<String>> {
                    override fun onSuccess(res: ResponseBody<String>?) {
                        AgoraLog.logMsg("$tag->publishStream state: streamUuid: ${stream.streamUuid}," +
                                ...
                                LogLevel.INFO.value)
                        val a = RteEngineImpl.setClientRole(eduRoom.getCurRoomUuid(), stream.streamUuid,
                                CLIENT_ROLE_BROADCASTER)
                        if (a != ok()) {
                            callback.onFailure(mediaError(a, getError(a)))
                            return
                        }
                        val b = RteEngineImpl.muteLocalStream(eduRoom.getCurRoomUuid(), stream.streamUuid,
                                !stream.hasAudio, !stream.hasVideo)
                        if (b != ok()) {
                            callback.onFailure(mediaError(b, getError(b)))
                            return
                        }
                        val c = RteEngineImpl.publish(eduRoom.getCurRoomUuid(), stream.streamUuid)
                        if (c != ok()) {
    ...
    private fun muteLocal(oldStream: EduStreamInfo, stream: EduStreamInfo): Int {
        ...
        if (oldStream.hasAudio != stream.hasAudio && oldStream.hasVideo != stream.hasVideo) {
            val code0 = RteEngineImpl.muteLocalAudioStream(eduRoom.getCurRoomUuid(), stream.streamUuid,
                    !stream.hasAudio)
            val code1 = RteEngineImpl.muteLocalVideoStream(eduRoom.getCurRoomUuid(), stream.streamUuid,
                    !stream.hasVideo)
            return if (code0 == ok() && code1 == ok()) ok() else -1
        } else if (oldStream.hasAudio != stream.hasAudio && oldStream.hasVideo == stream.hasVideo) {
            val code0 = RteEngineImpl.muteLocalAudioStream(eduRoom.getCurRoomUuid(), stream.streamUuid,
                    !stream.hasAudio)
            return if (code0 == ok()) ok() else -1
        } else {
            val code1 = RteEngineImpl.muteLocalVideoStream(eduRoom.getCurRoomUuid(), stream.streamUuid,
                    !stream.hasVideo)
            return if (code1 == ok()) ok() else -1
        }
    }
    ...
    override fun muteStream(stream: EduStreamInfo, callback: EduCallback<Boolean>) {
        ...
        if (index > -1) {
            ...
            if (oldStream == stream) {
                ...
                // If operate on local stream, change client role here
                if (stream.publisher.userUuid == userInfo.userUuid) {
                    val a = RteEngineImpl.setClientRole(
                        eduRoom.getCurRoomUuid(),
                        stream.streamUuid,
                        CLIENT_ROLE_BROADCASTER)
                ...
                RetrofitManager.instance()!!.getService(AgoraEduSDK.baseUrl(), StreamService::class.java)
                        .updateStreamInfo(APPID, eduRoom.getCurRoomUuid(), stream.publisher.userUuid,
                            stream.streamUuid, eduStreamStatusReq)
                        .enqueue(RetrofitManager.Callback(0, object : ThrowableCallback<ResponseBody<String>> {
                            override fun onSuccess(res: ResponseBody<String>?) {
        ...
        RetrofitManager.instance()!!.getService(AgoraEduSDK.baseUrl(), StreamService::class.java)
                ...
                .enqueue(RetrofitManager.Callback(0, object : ThrowableCallback<ResponseBody<String>> {
                    override fun onSuccess(res: ResponseBody<String>?) {
                        ...
                        val c = RteEngineImpl.unpublish(eduRoom.getCurRoomUuid(), stream.streamUuid)
                        if (c != ok()) {
                            callback.onFailure(mediaError(c, getError(c)))
                            return
                        }
...
//                }))
    ...
    override fun setStreamView(stream: EduStreamInfo, channelId: String, viewGroup: ViewGroup?,
                               config: RteRenderConfig, top: Boolean): EduError {
        if (TextUtils.isEmpty(stream.streamUuid)) {
        ...
        val curLocalUid = (userInfo.streamUuid.toLong() and 0xffffffffL).toInt()
        ...
        if (viewGroup == null) {
            ...
            videoCanvas = RteVideoCanvas(null, config.rteRenderMode.value, uid)
        } else {
            ...
            val textureView = RteEngine.createTextureView(
                    viewGroup.context.applicationContext)
            textureView.tag = stream.streamUuid
//            textureView.setZOrderMediaOverlay(top)
            ...
            addTextureView(viewGroup, textureView)
...
//                uid = 0
            ...
            videoCanvas = RteVideoCanvas(textureView, config.rteRenderMode.value,
                    config.rteMirrorMode.value, uid)
        }
        val code = if (uid == curLocalUid) {
            ...
            val result = RteEngineImpl.setupRemoteVideo(channelId, userInfo.streamUuid, videoCanvas)
            if (result == 0) {
    ...
    private fun addTextureView(parent: ViewGroup, textureView: TextureView) {
    ...
    override fun setStreamView(stream: EduStreamInfo, channelId: String, viewGroup: ViewGroup?, top: Boolean): EduError {
        ...
        val config = if (stream.videoSourceType == VideoSourceType.SCREEN) RteRenderMode.FIT else
            RteRenderMode.HIDDEN
        return setStreamView(stream, channelId, viewGroup, RteRenderConfig(config), top)
    }
    ...
    override fun resetVideoEncoderConfig(videoEncoderConfig: RteVideoEncoderConfig): EduError {
        val code = RteEngineImpl.setVideoEncoderConfiguration(videoEncoderConfig)
        return if (code == ok()) {
            noError()
        } else {
    ...
    override fun enableDualStreamMode(enabled: Boolean) {
        RteEngineImpl.enableDualStreamMode(enabled)
    }

    override fun setRemoteVideoStreamType(uid: String, streamType: RteVideoStreamType) {
        RteEngineImpl.setRemoteVideoStreamType(eduRoom.getCurRoomUuid(), uid, streamType)
    }
}
