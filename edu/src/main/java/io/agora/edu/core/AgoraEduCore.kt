...
import io.agora.edu.core.internal.edu.classroom.*
...
import io.agora.edu.core.internal.framework.data.EduError.Companion.internalError
...
class AgoraEduCore(
                ...
                joinRoomAsRole(
                    ...
                    object : EduCallback<EduLocalUser?> {
                        override fun onSuccess(res: EduLocalUser?) {
                            res?.let { localUser ->
                                AgoraLog.d("$tag-> join classroom success, " +
                        ...
                        override fun onFailure(error: EduError) {
                            AgoraLog.d("$tag-> join classroom fail, " +
    ...
    @Synchronized
    private fun joinRoomAsRole(name: String?, uuid: String?,
                               role: EduUserRole,
        ...
        val options = RoomJoinOptions(uuid!!, name, role,
            RoomMediaOptions(
                autoSubscribe, autoPublish,
                config.mediaOptions?.encryptionConfigs),
            config.roomType, config.videoEncoderConfig,
            config.latencyLevel.convert())
        ...
        eduRoom?.join(options, object : EduCallback<EduLocalUser> {
            override fun onSuccess(res: EduLocalUser?) {
                if (res != null) {
                    ...
                    reportClassJoinResult("0", error.type.toString() + "",
    ...
    private fun getReporter(): APaasReporter {
    ...
    private val mediaContextImpl: MediaContext = object : MediaContext() {
        ...
        override fun startPreview(container: ViewGroup) {
            AgoraLog.i("$tag:startPreview->container:$container")
            val videoTrack = eduManager?.getEduMediaControl()?.createCameraVideoTrack()
            val b = videoTrack?.setRenderConfig(RteRenderConfig(RteRenderMode.FIT))
            AgoraLog.i("$tag:setRenderConfig-fit->$b")
            val config = RteVideoEncoderConfig()
            ...
            AgoraLog.i("$tag:setVideoEncoderConfig->$c")
        ...
        override fun setVideoEncoderConfig(videoEncoderConfig: EduContextVideoEncoderConfig) {
            localUser?.resetVideoEncoderConfig(videoEncoderConfig.convert())
        }
    }
    ...
    private val userContextImpl: UserContext = object : UserContext() {
        override fun localUserInfo(): EduContextUserInfo {
            ...
            return EduContextUserInfo(
                    userUuid = uid,
                    userName = name,
                    role = EduContextUserRole.fromValue(config.roleType),
        ...
        override fun setVideoEncoderConfig(config: EduContextVideoEncoderConfig) {
            ...
            localUser?.resetVideoEncoderConfig(config.convert())
        }

        override fun muteRemoteVideo(streamUuid: String, muted: Boolean) {
            AgoraLog.i("$tag:userContextImpl-muteRemoteVideo->userUuid:$streamUuid,mute:$muted")
            userListManager?.muteRemoteVideo(streamUuid, muted)
        }

        override fun muteRemoteAudio(streamUuid: String, muted: Boolean) {
            AgoraLog.i("$tag:userContextImpl-muteRemoteAudio->userUuid:$streamUuid,mute:$muted")
            userListManager?.muteRemoteAudio(streamUuid, muted)
        }

        override fun updateCoHost(userUuid: String, coHost: Boolean) {
            AgoraLog.i("$tag:userContextImpl-updateCoHost->userUuid:$userUuid,coHost:$coHost")
            handsUpManager?.updateCoHost(userUuid, coHost)
        }

        override fun rewardUsers(userUuid: String, rewardCount: Int) {
            AgoraLog.i("$tag:userContextImpl-rewardUsers->userUuid:$userUuid,rewardCount:$rewardCount")
            handsUpManager?.reward(userUuid, rewardCount)
        }

        override fun updateBoardGranted(userUuid: String, granted: Boolean) {
            AgoraLog.i("$tag:userContextImpl-updateBoardGranted->userUuid:$userUuid,granted:$granted")
            if (granted) {
                whiteBoardManager?.grantBoardPermission(userUuid)
            } else {
                whiteBoardManager?.revokeBoardPermission(userUuid)
            }
        }

        override fun kick(userUuid: String, forever: Boolean) {
            handsUpManager?.kick(userUuid, forever)
        }
    }
    ...
    private val videoContextImpl: VideoContext = object : VideoContext() {
        override fun updateVideo(enabled: Boolean) {
            ...
            when (config.roomType) {
                ...
                AgoraEduRoomType.AgoraEduRoomTypeSmallArt.value -> {
                    teacherVideoManager?.muteLocalVideo(!enabled)
                }
        ...
        override fun joinWhiteboard() {
            ...
            synchronized(this@AgoraEduCore) {
                ...
                if (whiteBoardManager == null) {
                    AgoraLog.e("$tag -> whiteboard context needs to be initialized first")
                }
        ...
        override fun selectRoster(anchor: View) {
            userContextImpl.getHandlers()?.forEach { handler ->
                handler.onRoster(context, anchor,
                        when (config.roomType) {
                            AgoraEduRoomType.AgoraEduRoomTypeSmall.value -> {
                                RosterType.SmallClass.value()
                            }
                            AgoraEduRoomType.AgoraEduRoomTypeBig.value -> {
                                RosterType.LargeClass.value()
                            }
                            else -> null
                        })
            }
    ...
    private fun initWhiteboardManager(container: ViewGroup, whiteboardContext: WhiteboardContext) {
        synchronized(this@AgoraEduCore) {
            ...
            if (whiteBoardManager == null) {
                ...
                AgoraLog.w("$tag, initialize whiteboard with container, room uid ${config.roomUuid}")
            } else {
                AgoraLog.w("$tag, repeatedly initialize whiteboard")
            }
        }
    }
    ...
    private val extAppContext: ExtAppContext = object : ExtAppContext {
        ...
        private fun initExtAppManager(layout: RelativeLayout, config: AgoraEduCoreConfig) {
            ...
            extAppManager = object : AgoraExtAppManager(config.appId, context, layout,
                    config.roomUuid, eduRoom, eduContextPool) {
    ...
    private fun leave(exit: Boolean) {
        ...
        eduRoom?.leave(object : EduCallback<Unit> {
            override fun onSuccess(res: Unit?) {
                AgoraLog.e("$tag:leave EduRoom ${config.roomName} success")
                eduContextPool().roomContext()?.getHandlers()?.forEach {
    ...
    override fun onRoomPropertiesChanged(changedProperties: MutableMap<String, Any>,
        ...
        AgoraLog.i("$tag:onRoomPropertiesChanged->changedProperties" +
                ":${Gson().toJson(changedProperties)}, " +
                "cause:${Gson().toJson(cause)}, operator:${Gson().toJson(operator)}, " +
                "room: ${config.roomUuid}")
    ...
    override fun onRemoteVideoStateChanged(channelId: String, uid: Int, state: Int, reason: Int, elapsed: Int) {
        AgoraLog.i("$tag:onRemoteVideoStateChanged->rtcChannel:${channelId}, uid:$uid, " +
    ...
    override fun onRemoteAudioStateChanged(channelId: String, uid: Int, state: Int, reason: Int, elapsed: Int) {
        AgoraLog.i("$tag:onRemoteAudioStateChanged->rtcChannel:${channelId}, uid:$uid, " +
                "state:$state, reason:$reason, elapsed:$elapsed")
        when (config.roomType) {
            AgoraEduRoomType.AgoraEduRoomType1V1.value -> {
                oneToOneVideoManager?.notifyUserDetailInfo(EduUserRole.TEACHER)
    ...
    override fun onAudioVolumeIndicationOfLocalSpeaker(speakers: Array<out RteAudioVolumeInfo>?, totalVolume: Int) {
        updateVolumes(speakers)
    }

    override fun onAudioVolumeIndicationOfRemoteSpeaker(speakers: Array<out RteAudioVolumeInfo>?, totalVolume: Int) {
        updateVolumes(speakers)
    }

    private fun updateVolumes(speakers: Array<out RteAudioVolumeInfo>?) {
        when (config.roomType) {
            AgoraEduRoomType.AgoraEduRoomType1V1.value -> {
    ...
    override fun onLocalUserUpdated(userEvent: EduUserEvent, type: EduUserStateChangeType) {
        AgoraLog.e("$tag:onLocalUserUpdated->userEvent:${Gson().toJson(userEvent)}, " +
                "type: ${type.name}")
    }

    override fun onLocalStreamAdded(streamEvent: EduStreamEvent) {
        AgoraLog.e("$tag:onLocalStreamAdded->streamEvent:${Gson().toJson(streamEvent)}")
        when (config.roomType) {
            AgoraEduRoomType.AgoraEduRoomType1V1.value -> {
    ...
    override fun onLocalStreamUpdated(streamEvent: EduStreamEvent) {
        AgoraLog.e("$tag:onLocalStreamUpdated->streamEvent:${Gson().toJson(streamEvent)}")
        when (config.roomType) {
            AgoraEduRoomType.AgoraEduRoomType1V1.value -> {
    ...
    override fun onLocalStreamRemoved(streamEvent: EduStreamEvent) {
        AgoraLog.e("$tag:onLocalStreamRemoved->streamEvent:${Gson().toJson(streamEvent)}")
        when (config.roomType) {
            AgoraEduRoomType.AgoraEduRoomType1V1.value -> {
    ...
    override fun onLocalUserLeft(userEvent: EduUserEvent, leftType: EduUserLeftType) {
        AgoraLog.e("$tag:onLocalUserLeft->userEvent:${Gson().toJson(userEvent)}, " +
...
data class AgoraEduCoreConfig(
        ...
        val agoraEduStreamState: AgoraEduStreamState?,
 ...
 */
internal object ClassInfoCache {
