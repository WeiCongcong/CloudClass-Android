...
import io.agora.edu.uikit.impl.chat.AgoraUIChatWidgetPopup
import io.agora.edu.uikit.impl.chat.EaseChatWidget
import io.agora.edu.uikit.impl.chat.EaseChatWidgetPopup
...
object AgoraEduSDK {
    ...
    fun launch(context: Context,
        ...
        injectChatWidgetByRoomType(config.roomType, config.region)
    ...
    private fun checkAndInit(context: Context, config: AgoraEduLaunchConfig) {
        ...
        val req = RoomPreCheckReq(
                config.roomName,
                config.roomType, config.roleType.toString(),//set role type
                config.startTime,
                config.duration,
                config.userName,
                config.agoraEduStreamState,
    ...
    private fun injectChatWidgetByRoomType(roomType: Int, region: String) {
        if (roomType == AgoraEduRoomType.AgoraEduRoomTypeSmallArt.value) {
            val configCn = mutableListOf(UiWidgetConfig(
                UiWidgetManager.DefaultWidgetId.Chat.name,
                EaseChatWidgetPopup::class.java))

            val configOther = mutableListOf(UiWidgetConfig(
                UiWidgetManager.DefaultWidgetId.Chat.name,
                AgoraUIChatWidgetPopup::class.java))

            if (region == AgoraEduRegion.cn) {
                registerAndReplace(region, configCn)
            } else {
                registerAndReplace(region, configOther)
            }
        } else {
            val configCn = mutableListOf(UiWidgetConfig(
                UiWidgetManager.DefaultWidgetId.Chat.name,
