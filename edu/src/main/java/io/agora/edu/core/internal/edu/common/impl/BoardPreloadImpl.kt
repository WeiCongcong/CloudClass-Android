...
import io.agora.edu.core.internal.download.DownloadConstant.EXTRA_INTENT_DOWNLOAD
import io.agora.edu.core.internal.download.DownloadHelper
...
import io.agora.edu.core.internal.edu.common.bean.board.BoardPreloadFileInfo
import io.agora.edu.core.internal.edu.common.listener.BoardPreloadListener
import io.agora.edu.core.internal.util.AppUtil
import io.agora.edu.core.internal.log.ZipUtils
import java.io.File
...
class BoardPreloadImpl(context: Context) : BoardPreload {
    ...
    override fun checkCache(request: WebResourceRequest): WebResourceResponse? {
        if (isSourceUrl(request.url.toString())) {
            var destPath = curDestFolder
            destPath = destPath.plus(File.separatorChar)
    ...
    private fun isSourceUrl(url: String): Boolean {
        return url.contains(publicFiles) || url.contains(dynamicConvert) || url.contains(staticConvert)
    }
}