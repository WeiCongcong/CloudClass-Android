    ...
    private fun dispatchChannelMsg(text: String) {
        ...
        when (cmdResponseBody.cmd) {
            ...
            CMDId.ChannelMsgReceived.value -> {
                ...
                val eduMsg = CMDUtil.buildEduMsg(text, eduRoom) as EduChatMessage
            ...
            CMDId.UserJoinOrLeave.value -> {
                ...
                validOfflineLocalUser

                validOfflineLocalUser?.let { info ->
                    AgoraLog.i("$TAG->onLocalUserRemoved:${Gson().toJson(info)}")
                    Log.w(TAG, " -- get user offline message!")

                    cmdCallbackManager.onLocalUserRemoved(info, eduRoom.getCurLocalUser(), rtmInOutMsg.offlineUsers.find { item ->
            ...
            CMDId.StreamStateChange.value, CMDId.StreamsStateChange.value -> {
                ...
                // record last streams, for compare.
                var oldStreams = mutableListOf<EduStreamInfo>()
                            ...
                            eduRoom.getCurLocalUser().initOrUpdateLocalStream(options,
                                    object : EduCallback<EduStreamInfo> {
                                        override fun onSuccess(res: EduStreamInfo?) {
                                            RteEngineImpl.muteLocalStream(eduRoom.getCurRoomUuid(),
                                                    streamInfo.streamUuid, !streamInfo.hasAudio,
                                                    !streamInfo.hasVideo)
                                            RteEngineImpl.setClientRole(eduRoom.getCurRoomUuid(),
                                                    streamInfo.streamUuid, CLIENT_ROLE_BROADCASTER)
                                            RteEngineImpl.publish(eduRoom.getCurRoomUuid(), streamInfo.streamUuid)
                                        }
                                ...
                                if (oldStreams.size == 0) {
                                    ...
                                    oldStreams.find { it.streamUuid == streamInfo.streamUuid }?.let {
                                        if (streamInfo.hasAudio != it.hasAudio) {//update hasAudio when it is not the same with the old one
                                            RteEngineImpl.updateLocalAudioStream(eduRoom.getCurRoomUuid(),
                                                    streamInfo.streamUuid, streamInfo.hasAudio)
                                        }

                                        if (streamInfo.hasVideo != it.hasVideo) {
                                            RteEngineImpl.updateLocalVideoStream(eduRoom.getCurRoomUuid(),
                                                    streamInfo.streamUuid, streamInfo.hasVideo)
                                        }
                                    }
                                }
                                RteEngineImpl.setClientRole(eduRoom.getCurRoomUuid(),
                        ...
                        if (streamInfo.publisher == eduRoom.getCurLocalUserInfo()) {
                            ...
                            eduRoom.getCurLocalUser().initOrUpdateLocalStream(options,
                                    object : EduCallback<EduStreamInfo> {
                                        override fun onSuccess(res: EduStreamInfo?) {
                                            RteEngineImpl.muteLocalStream(eduRoom.getCurRoomUuid(),
                                                    streamInfo.streamUuid, false, false)
                                            RteEngineImpl.setClientRole(eduRoom.getCurRoomUuid(),
                                                    streamInfo.streamUuid, CLIENT_ROLE_AUDIENCE)
