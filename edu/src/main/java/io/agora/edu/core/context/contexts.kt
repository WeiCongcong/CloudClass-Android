...
abstract class UserContext : AbsHandlerPool<IUserHandler>() {
    ...
    // 控制远端视频mute状态
    abstract fun muteRemoteVideo(streamUuid: String, muted: Boolean)

    // 控制远端音频mute状态
    abstract fun muteRemoteAudio(streamUuid: String, muted: Boolean)

    // 指定上/下台学生
    abstract fun updateCoHost(userUuid: String, coHost: Boolean)

    // 发奖
    abstract fun rewardUsers(userUuid: String, rewardCount: Int)

    // 控制白板授权
    abstract fun updateBoardGranted(userUuid: String, granted: Boolean)

    abstract fun kick(userUuid: String, forever: Boolean)
}
