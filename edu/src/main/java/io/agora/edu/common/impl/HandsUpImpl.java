...
import io.agora.edu.common.bean.handsup.HandsupBody;
...
public class HandsUpImpl extends Base implements HandsUp {
    ...
    @Override
    public void applyHandsUp(@NotNull EduCallback<Boolean> callback, int timeout) {
        HandsupBody handsupBody = new HandsupBody(timeout);
        RetrofitManager.instance().getService(AgoraEduSDK.baseUrl(), HandsUpService.class)
                .applyHandsUp(appId, roomUuid, handsupBody)
    ...
    @Override
    public void applyHandsWave(@NotNull EduCallback<Boolean> callback, int timeout) {
        HandsupBody handsupBody = new HandsupBody(timeout);
        RetrofitManager.instance().getService(AgoraEduSDK.baseUrl(), HandsUpService.class)
                .applyHandsWave(appId, roomUuid, handsupBody)
                .enqueue(new RetrofitManager.Callback(0, new ThrowableCallback<ResponseBody<String>>() {
                    @Override
                    public void onFailure(@Nullable Throwable throwable) {
                        if (throwable instanceof BusinessException) {
