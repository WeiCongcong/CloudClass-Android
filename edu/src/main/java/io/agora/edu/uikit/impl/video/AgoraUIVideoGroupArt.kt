...
class AgoraUIVideoGroupArt(
        ...
        height: Int,
        margin: Int,
        mode: io.agora.edu.core.context.EduContextVideoMode = io.agora.edu.core.context.EduContextVideoMode.Single) : AbsComponent(), IAgoraUIVideoListener {
    ...
    private var remoteUserDetailInfo: io.agora.edu.core.context.EduContextUserDetailInfo? = null
    private var localUserDetailInfo: io.agora.edu.core.context.EduContextUserDetailInfo? = null
    ...
    private val videoGroupHandler = object : VideoHandler() {
        override fun onUserDetailInfoUpdated(info: io.agora.edu.core.context.EduContextUserDetailInfo) {
            if (info.user.role == io.agora.edu.core.context.EduContextUserRole.Teacher) {
                remoteUserDetailInfo = info
                remoteVideo?.upsertUserDetailInfo(info)
            } else if (info.user.role == io.agora.edu.core.context.EduContextUserRole.Student) {
    ...
    init {
        ...
        remoteVideo = AgoraUIVideoArt(context, remoteLayout, 0.0f, 0.0f, 0f)
        remoteVideo?.videoListener = this

        if (mode == io.agora.edu.core.context.EduContextVideoMode.Pair) {
            ...
            localVideo = AgoraUIVideoArt(context, localLayout, 0.0f, 0.0f, 0f)
    ...
    fun updateUserDetailInfo(info: io.agora.edu.core.context.EduContextUserDetailInfo) {
