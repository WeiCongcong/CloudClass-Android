...
import io.agora.edu.uikit.impl.room.AgoraUIRoomStatusNormal
...
class AgoraUISmallClassContainer(
    ...
    override fun init(layout: ViewGroup, left: Int, top: Int, width: Int, height: Int) {
        ...
        roomStatus = AgoraUIRoomStatusNormal(layout, getEduContext(), width, statusBarHeight, left, top)
        ...
        chat = widgetManager.create(UiWidgetManager.DefaultWidgetId.Chat.name, getEduContext()) as? ChatWidget
        chat?.let {
        ...
        handsUpFullScreenRect.set(chatFullScreenHideLeft - componentMargin - handsUpWidth, handsUpTop,
                chatFullScreenHideLeft - componentMargin, handsUpTop + handsUpHeight)
...
//                it.showShadow(false)
                ...
                it.setRect(if (it.isShowing()) chatRect else chatFullScreenHideRect)
            }
